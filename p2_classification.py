    ...
    for key in classification_reports[0].keys():
        if isinstance(classification_reports[0][key], dict):
            ...
            for metric in classification_reports[0][key].keys():
                mean_report[key][metric] = np.mean(
                    [report[key][metric] for report in classification_reports if isinstance(report[key], dict)])
        else:
...
def print_results(accuracies, confusion_matrices, classification_reports, f1_scores, start_time, end_time):
    ...
    print(
        f"Mean F1-score for Bankrupt class: {np.mean(f1_scores):.2f}, with Standard Deviation of {np.std(f1_scores):.2f}\n")
...
def main():
    ...
    accuracies, confusion_matrices, classification_reports, f1_scores, feature_importances_list = train_predict(X, y,
                                                                                                                model,
                                                                                                                iterations=ITERATIONS,
                                                                                                                train_size=0.8,
                                                                                                                permutation_importance_flag=True)
    end_time = time.time()
    ...
    start_time = time.time()
    accuracies, confusion_matrices, classification_reports, f1_scores, _ = train_predict(X, y, model,
                                                                                         iterations=ITERATIONS,
                                                                                         train_size=0.8,
